...
> 本项目 fork 自 [phuryn/grok-build-vscode](https://github.com/phuryn/grok-build-vscode)（Paweł Huryn，MIT），在其成熟的 Grok Build 侧边栏之上增加了四项差异化能力：
>
> 1. **BYOK 原生 Chat 集成** — `grok-4.5` 直接出现在 VS Code 原生 Chat 模型选择器中（Grok 账号 OAuth 授权或 xAI API Key，独立于 CLI 登录），并附带 `@grok` 参与者（/fix /explain /review /docs /search /commit）
> 2. **SCM 一键提交信息** — Git 面板 ✨ 按钮，读 diff 流式生成 Conventional Commits 提交信息
> 3. **状态栏指示器** — 当前模型/推理强度一目了然（上游 roadmap 未实现项）
> 4. **中文界面** — 命令与设置已本地化（跟随 VS Code 显示语言）
>
> 直连 API 模块的说明见 [docs/direct-mode.md](docs/direct-mode.md)。以下为上游原版 README。
