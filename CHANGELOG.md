...
- 发布前整理首发包元数据与文档，补齐 `0.3.1` 打包发布记录。

## 0.3.0 (Grok Coder fork) — 2026-07-09

基于上游 grok-build-vscode 1.4.30（Paweł Huryn，MIT）的 fork 首个版本。

### Added

- **BYOK Provider**：grok-4.5 注册进 VS Code 原生 Chat 模型选择器（Grok 账号 OAuth / xAI API Key 双轨认证，Responses API 直连），含 `@grok` 聊天参与者与六个 slash 命令（src/direct/）
- **SCM 一键提交信息**：Git 面板 ✨ 按钮，流式生成 Conventional Commits（src/direct/commit.ts）
- **状态栏指示器**：显示当前模型/推理强度，点击弹出快捷菜单（src/direct/statusbar.ts）
- **中文本地化**：命令与设置 l10n（package.nls.zh-cn.json）

### Changed

- 品牌与 ID：`brucelee.grok-coder`；engines ≥ 1.104
- 遥测：不再向上游作者的 Aptabase 项目发送（发送点已断开，待注册自有项目）

---

以下为上游 grok-build-vscode 的原始变更记录。
